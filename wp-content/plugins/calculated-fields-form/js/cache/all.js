...
{fbuilderjQuery['fbuilder']['extend_window'](fbuilderjQuery['fbuilder']['modules']['cookies']['prefix'],CF_COOKIES);}};$.fbuilder['version']='1.2.54';$.fbuilder['controls']=$.fbuilder['controls']||{};$.fbuilder['forms']=$.fbuilder['forms']||{};$.fbuilder['isNumeric']=function(n){return!isNaN(parseFloat(n))&&isFinite(n);};$.fbuilder['htmlEncode']=window['cff_esc_attr']=function(value)
...
fieldlist_tag.find(".pbPrevious,.pbNext").on("keyup",function(evt){if(evt.which==13||evt.which==32)$(this).click();}).on("click",{'identifier':opt.identifier},function(evt){var _from=($.fbuilder.forms[evt.data.identifier]['currentPage']||0),_inc=($(this).hasClass("pbPrevious"))?-1:1,_p=$.fbuilder['goToPage']({'formIdentifier':evt.data.identifier,'from':_from,'to':_from+_inc,'callback':function()
...
jQuery(document).on('click','.cff-help-icon',function(evt){evt.stopPropagation();evt.preventDefault();});$(document).on('click','#fbuilder .captcha img',function(){try{var e=$(this),src=e.attr('src');if(!(new RegExp('^http(s)?\:\/\/'+$.fbuilder.escapeSymbol(window.location.host),'i')).test(src))src=document.location.href.split('?')[0]+'?'+src.split('?')[1];e.attr('src',src.replace(/&\d+$/,'')+'&'+Math.floor(Math.random()*1000));}catch(err){if('console' in window)console.log(err);}});$(form_tag).find('.captcha img').click();$('#fieldlist'+opt.identifier).find(".pbSubmit").off('click').on("keyup",function(evt){if(evt.which==13||evt.which==32)$(this).click();}).on("click",{'identifier':opt.identifier},function(evt){$(this).closest("form").submit();});if(i>0)
...
{var me=this,v=String(me[attr]).trim();if($.fbuilder.isNumeric(v))return;var s=(/^fieldname\d+$/i.test(v))?'[id*="'+v+me.form_identifier+'"]':v,i=(one)?'one':'on';if('string'==typeof s&&!/^\s*$/.test(s))
...
tags.each(function(){$(this).html(value);});}}catch(err){}};$(document).on('change keyup','#fbuilder :input[name*="fieldname"]',function(){var me=this;setTimeout(function(){assign_data_cff_field_content(me);},50);});$(document).on('keyup','#fbuilder :input[maxlength]',function(){var e=$(this),v=new String(e.val()),l=v.length,m=e.attr('maxlength');if(m*1<=l){setTimeout(function(){e.val(v.substring(0,m)).change();},5);}});$(document).one('showHideDepEvent',function(){try{var form_obj=$('#'+arguments[1]);form_obj.find('[data-cff-field]').each(function(){try{var tag_obj=$(this),field_name=tag_obj.attr('data-cff-field'),field_obj;if(field_name.length&&/fieldname\d+/.test(field_name)){field_obj=form_obj.find('[name*="'+field_name+'_"]');if(field_obj.length)assign_data_cff_field_content(field_obj);}}catch(err){}});}catch(err){}});$.fbuilder.controls['ftext']=function(){};$.extend($.fbuilder.controls['ftext'].prototype,$.fbuilder.controls['ffields'].prototype,{title:"Untitled",ftype:"ftext",autocomplete:"off",predefined:"",predefinedClick:false,required:false,readonly:false,size:"medium",minlength:"",maxlength:"",equalTo:"",regExp:"",regExpMssg:"",show:function()
...
{var e=$('[id*="'+this.name+'"].hasDatepicker'),f;if(e.length)
{e.datepicker('option','minDate',(ignore)?null:v);if(e.has('.datepicker-container')){f=e;e=e.siblings('.date-component');}
if(e.val()!='')e.change();else if(f)f.find('.ui-state-active').removeClass('ui-state-active');}},set_maxDate:function(v,ignore)
{var e=$('[id*="'+this.name+'"].hasDatepicker'),f;if(e.length)
{e.datepicker('option','maxDate',(ignore)?null:v);if(e.has('.datepicker-container')){f=e;e=e.siblings('.date-component');}
if(e.val()!='')e.change();else if(f)f.find('.ui-state-active').removeClass('ui-state-active');}},set_DefaultDate:function(init)
{var me=this,p={dateFormat:me.dformat.replace(/yyyy/g,"yy"),minDate:me._getAttr('minDate'),maxDate:me._getAttr('maxDate'),firstDay:(me.mondayFirstDay?1:0),disabled:me.readonly},dp=$("#"+me.name+"_date"),dd=me.currentDate&&init?new Date():((me.defaultDate!="")?me.defaultDate:((me.predefined!="")?me.predefined:''));if(me.alwaysVisible&&me.showDatepicker){dp=$("#"+me.name+"_datepicker_container");p['altField']=$("#"+me.name+"_date");p['altFormat']=p['dateFormat'];p['onSelect']=function(dateText,inst){$("#"+me.name+"_date").change();};}
...
{var m=this,tmp;$(document).off('click','[id*="'+m.name+'"]').on('click','[id*="'+m.name+'"]',function(){m.enable_disable();});m.enable_disable();if(m.readonly){$('[id*="'+m.name+'"][_onclick]').each(function(){$(this).attr('onclick',$(this).attr('_onclick'));});}
if(0<m.max&&0<m.min&&m.max<m.min){tmp=m.min;m.min=m.max;m.max=tmp;}
if(0<m.max)
$('[id*="'+m.name+'"]').rules('add',{maxlength:m.max,messages:{maxlength:m.maxError}});if(0<m.min)
$('[id*="'+m.name+'"]').rules('add',{minlength:m.min,messages:{minlength:m.minError}});},showHideDep:function(toShow,toHide,hiddenByContainer,interval)
...
if(me.readonly){$('[id*="'+me.name+'"][_onclick]').each(function(){$(this).attr('onclick',$(this).attr('_onclick'));});}},showHideDep:function(toShow,toHide,hiddenByContainer,interval)
{if(typeof hiddenByContainer=='undefined')hiddenByContainer={};var me=this,item=$('input[id*="'+me.name+'"]'),form_identifier=me.form_identifier,isHidden=(typeof toHide[me.name]!='undefined'||typeof hiddenByContainer[me.name]!='undefined'),result=[];try
{item.each(function(i,e){if(typeof me.choicesDep[i]!='undefined'&&me.choicesDep[i].length)
{var checked=e.checked;for(var j=0,k=me.choicesDep[i].length;j<k;j++)
{if(!/fieldname/i.test(me.choicesDep[i][j]))continue;var dep=me.choicesDep[i][j]+form_identifier;if(isHidden||!checked)
...
$('#'+this.id+'_clearer').after(filesContainer);}});$('#'+me.name+'_clearer').on('click',function(){$('#'+me.name).val('').change().valid();});},val:function(raw,no_quotes)
...
{$('#'+me.name+'_'+i).on('change',function(){var v='';$('[id*="'+me.name+'_"]').each(function(){v+=$(this).val();});$('#'+me.name).val(v).change();});if(i+1<h)
...
{for(var i=0;i<l;i++)
{if(!/^\s*$/.test(p[i]))
{p[i]=String(p[i]).trim()+me.form_identifier;if($('[class*="'+p[i]+'"]').length){str+='<div ref="'+p[i]+'" class="cff-summary-item"><span class="'+cff_esc_attr(me.titleClassname)+' cff-summary-title"></span><span class="'+cff_esc_attr(me.valueClassname)+' cff-summary-value"></span></div>';me.fieldsArray.push(p[i]);$(document).on('change','[id*="'+p[i]+'"]',function(){me.update();});}}}
...
{var me=this;$('#'+this.name).mousedown(function(){eval(me.sOnmousedown);});$('#'+this.name).on('click',function()
