...
if (!is_plugin_active('Ultimate-Premium-Plugin/usm_premium_icons.php')) {
	...
	define( 'SFSI_PLUS_PLUGIN_VERSION', '3.6.2' );
	...
	include(SFSI_PLUS_DOCROOT . '/libs/controllers/sfsi_forum_notification_popup.php');
	...
	if (!get_option('sfsi_plus_pluginVersion') || get_option('sfsi_plus_pluginVersion') < 3.52) {
	...
	function sfsi_plus_beforaftereposts($content)
	{
		...
		if (is_single()) {
			...
			if ( $option8['sfsi_plus_display_before_posts'] == "yes" && $option8['sfsi_plus_show_item_onposts'] == "yes" && $option8['sfsi_plus_display_button_type'] !== 'responsive_button' ) {
				...
				if ($sfsi_plus_display_button_type == 'standard_buttons') {
					...
					$icons_before .= "<div style='float:left;margin:0; line-height:" . $lineheight . "px'><span>" . $txt . "</span></div>";
			...
			if ($option8['sfsi_plus_show_item_onposts'] == "yes") {
				...
				if ($option8['sfsi_plus_display_after_posts'] == "yes") {
					if ($sfsi_plus_display_button_type == 'standard_buttons') {
						...
						$icons_after .= "<div style='float:left;margin:0; line-height:" . $lineheight . "px'><span>" . $txt . "</span></div>";
	...
	function sfsi_plus_beforeafterblogposts($content)
	{
		if (is_home()) {
			...
			if (
				...
				$icons_before .= "<div class='sfsibeforpstwpr' style='" . $style_parent . "'>";
				$icons_before .= "<div class='sfsi_plus_Sicons " . $float .$mouse_hover_effect. "' style='" . $style . "'>";
				if ($sfsi_plus_display_button_type == 'standard_buttons') {
					if (
						...
						$icons_before .= "<div class='sfsi_plus_Sicons_text_wrapper' style='display: inline-block;margin-bottom: 0; margin-left: 0; margin-right: 8px; margin-top: 0; vertical-align: middle;width: auto;'><span>" . $txt . "</span></div>";
					}
					...
					if ($sfsi_section8['sfsi_plus_recttwtr'] == 'yes') {
						...
						$icons_before .=  sfsi_twitterShare( $permalink, $post_title );
					}
					if ($sfsi_section8['sfsi_plus_rectpinit'] == 'yes') {

						$icons_before .= "<div class='sf_pinit' style='display: inline-block;vertical-align: middle;text-align:left;'>" . sfsi_plus_pinterest_Custom($permalink, $show_count) . "</div>";
					}
			...
			if ($sfsi_section8['sfsi_plus_show_item_onposts'] == "yes") {
				...
				//icon selection
				$icons_after .= "<div class='sfsiaftrpstwpr' style='" . $style_parent . "'>";
				$icons_after .= "<div class='sfsi_plus_Sicons " . $float . $mouse_hover_effect."' style='" . $style . "'>";
				...
				if ($sfsi_section8['sfsi_plus_display_after_blogposts'] == "yes") {
					...
					if ($sfsi_plus_display_button_type == 'standard_buttons') {
						if (
							...
							$icons_after .= "<div class='sfsi_plus_Sicons_text_wrapper' style='display: inline-block;margin-bottom: 0; margin-left: 0; margin-right: 8px; margin-top: 0; vertical-align: middle;width: auto;'><span>" . $txt . "</span></div>";
						}
						if ($sfsi_section8['sfsi_plus_rectsub'] == 'yes') {
							if ($show_count) {
								$sfsiLikeWithsub = "93px";
						...
						if ($sfsi_section8['sfsi_plus_recttwtr'] == 'yes') {
							...
							$icons_after .= sfsi_twitterShare( $permalink, $post_title );
						}
						if ($sfsi_section8['sfsi_plus_rectpinit'] == 'yes') {
							$icons_after .= "<div class='sf_pinit' style='display: inline-block;text-align:left;vertical-align: middle;'>" . sfsi_plus_pinterest_Custom($permalink, $show_count) . "</div>";
						}
					} else if ($sfsi_section8['sfsi_plus_display_button_type'] == 'responsive_button') {
						// if (isset($sfsi_section8['sfsi_plus_responsive_icons_end_post']) && $sfsi_section8['sfsi_plus_responsive_icons_end_post'] == "yes") {
	...
	function sfsi_plus_admin_notice()
	{
		...
		<?php
			...
			<div class="updated sfsi_plus_show_prem_notification" style="<?php echo $style; ?>background-color: #38B54A; color: #fff; font-size: 18px;">
				<div class="alignleft" style="margin: 9px 0;">
					<?php _e('BIG NEWS : There is now a Premium Ultimate Social Media Plugin available with many more cool features: ', 'ultimate-social-media-plus'); ?><a href="https://www.ultimatelysocial.com/usm-premium/?utm_source=usmplus_settings_page&utm_campaign=notification_banner&utm_medium=banner" target="_blank"><?php _e('Check it out', 'ultimate-social-media-plus'); ?></a>
				</div>
			...
			<?php
				...
				<!-- <div class="updated sfsi_plus_show_mobile_setting_notification" style="<?php //echo $style;
							...
							<b><?php //_e( 'Over 50% of visitors are mobile visitors:', 'ultimate-social-media-plus');
												?></b>
							<?php //_e( ' Make sure your social media icons look good on mobile too, so that people like & share your site. With the premium plugin you can define the location of the icons separately on mobile: ', 'ultimate-social-media-plus');
											?><a href="https://www.ultimatelysocial.com/usmpremium/?utm_expid=92383224-1.TfahStjhTrSpmi_nxkXt1w.1&utm_source=usmplus_settings_page&utm_campaign=check_mobile&utm_medium=banner" target="_blank"><?php //_e( 'Check it out', 'ultimate-social-media-plus');
																																																																?></a>
							</div>
							<div class="alignright">
				...
				<div class="updated sfsi_plus_show_phperror_notification" style="<?php echo $style; ?>background-color: #D22B2F; color: #fff; font-size: 18px; border-left-color: #D22B2F;">
					<div class="alignleft" style="margin: 9px 0;">
						<p class="sfsi_plus_show_notifictaionpragraph">
							<?php _e('We noticed you are running your site on a PHP version older than 5.6. Please upgrade to a more recent version. This is not only important for running the Ultimate Social Media Plugin, but also for security reasons in general.', 'ultimate-social-media-plus'); ?>
							<br>
            ...
            Spanish (Peru), Spanish (Puerto Rico), Spanish (Spain), Spanish (Venezuela) */
				...
				function sfsi_plus_language_notice()
				{
					...
					<div id="sfsi_plus_langnotice" class="notice notice-info">
						...
						<p><?php _e($langText, 'ultimate-social-media-plus'); ?></p>
			...
			<div class="sfsi_plus_askforhelp askhelpInview<?php echo $viewNumber; ?>">
				...
				<img src="<?php echo SFSI_PLUS_PLUGURL . "images/questionmark.png" ?>" alt="" />
				<span><?php
					printf(
						__( 'Have issues? %1$sAsk us in the forum%2$s - %3$sit\'s FREE and quick!', 'ultimate-social-media-plus' ),
				...
				<?php endif;
					...
					function sfsi_plus_addThis_removal_notice()
					{
						...
						if (isset($_GET['page']) && "sfsi-plus-options" == $_GET['page']) :
							...
							$sfsi_plus_addThis_removalText    = __("We removed Addthis from the plugin due to issues with GDPR, the new EU data protection regulation.", 'ultimate-social-media-plus');
		...
		<?php
			...
			add_action('wp_ajax_wpse1_6817_btn', function () {
				...
				if (check_ajax_referer('wps18472_installation', 'nonce', false) === false) {
			...
			function sfsi_plus_mobile_icons_shown()
			{
				...
				$sfsi_plus_section5            =  maybe_unserialize(get_option('sfsi_plus_section5_options', false));
      ...
      add_action('plugins_loaded', function () {
        ...
        if (!(class_exists('\Inisev\Subs\Inisev_Try_Out_Plugins') || class_exists('Inisev\Subs\Inisev_Try_Out_Plugins') || class_exists('Inisev_Try_Out_Plugins'))) {
          ...
          $try_out_plugins = new \Inisev\Subs\Inisev_Try_Out_Plugins(__FILE__, __DIR__, 'Ultimate Social Media Plus', 'admin.php?page=sfsi-plus-options');
        }
