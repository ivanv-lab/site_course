...
function sfsi_plus_update_plugin()
{
	...
	//Install version
	update_option("sfsi_plus_pluginVersion", "3.53");
	...
	if(in_array($lastversion,$sfsi_banner_error_version)){
	    ...
	    $sfsi_plus_dismiss_sharecount = maybe_unserialize(get_option('sfsi_plus_dismiss_sharecount'));
	    ...
	    $sfsi_plus_dismiss_google_analytic = maybe_unserialize(get_option('sfsi_plus_dismiss_google_analytic'));
	    ...
	    $sfsi_plus_banner_global_firsttime_offer = maybe_unserialize(get_option('sfsi_plus_banner_global_firsttime_offer'));
	    ...
	    $sfsi_plus_dismiss_gdpr = maybe_unserialize(get_option('sfsi_plus_dismiss_gdpr'));
	    ...
	    $sfsi_plus_dismiss_optimization = maybe_unserialize(get_option('sfsi_plus_dismiss_optimization'));
	    ...
	    $sfsi_plus_dismiss_gallery = maybe_unserialize(get_option('sfsi_plus_dismiss_gallery'));
		...
		$sfsi_plus_dismiss_woocommerce = maybe_unserialize(get_option('sfsi_plus_dismiss_woocommerce'));
	    if (!isset($sfsi_plus_dismiss_woocommerce) || empty($sfsi_plus_dismiss_woocommerce)) {
		...
		$sfsi_plus_dismiss_twitter = maybe_unserialize(get_option('sfsi_plus_dismiss_twitter'));
	    if (!isset($sfsi_plus_dismiss_twitter) || empty($sfsi_plus_dismiss_twitter)) {
		...
		$sfsi_plus_dismiss_copy_delete_post = maybe_unserialize(get_option('sfsi_plus_dismiss_copy_delete_post'));
	    ...
	    $sfsi_plus_banner_global_upgrade =  maybe_unserialize(get_option('sfsi_plus_banner_global_upgrade'));
	    ...
	    $sfsi_plus_banner_global_http =  maybe_unserialize(get_option('sfsi_plus_banner_global_http'));
	    ...
	    $sfsi_plus_banner_global_gdpr =  maybe_unserialize(get_option('sfsi_plus_banner_global_gdpr'));
	    ...
	    $sfsi_plus_banner_global_shares =  maybe_unserialize(get_option('sfsi_plus_banner_global_shares'));
	    ...
	    $sfsi_plus_banner_global_load_faster =  maybe_unserialize(get_option('sfsi_plus_banner_global_load_faster'));
	    ...
	    $sfsi_plus_banner_global_social =  maybe_unserialize(get_option('sfsi_plus_banner_global_social'));
	    ...
	    $sfsi_plus_banner_global_pinterest =  maybe_unserialize(get_option('sfsi_plus_banner_global_pinterest'));
	...
	update_option('sfsi_plus_currentDate',  date('Y-m-d h:i:s'));
	update_option('sfsi_plus_showNextBannerDate', '14 day');
    // if (get_option('sfsi_plus_showNextBannerDate') == "21 day") {
    //     update_option('sfsi_plus_showNextBannerDate', '14 day');
    // }
    update_option('sfsi_plus_cycleDate',  "180 day");
	update_option('sfsi_plus_loyaltyDate',  "180 day");
	...
	//New
	update_option("sfsi_plus_was_installed_before", SFSI_PLUS_PLUGIN_VERSION);
}
function sfsi_plus_activate_plugin()
{
    ...
    $options2=array('sfsi_plus_rss_url'=>sfsi_plus_get_bloginfo('rss2_url'),
        ...
        'sfsi_plus_twitter_aboutPageText'=>'Hey, check out this cool site I found: www.yourname.com #Topic via@my_x_twitter_name',
    ...
    $options5=array('sfsi_plus_icons_size'=>'40',
        ...
        'sfsi_plus_twitter_MouseOverText'=>'X (Twitter)',
		...
		'sfsi_plus_show_admin_popup'=> 'yes',
	...
	/*Changes in option 2*/
	$get_option2 = maybe_unserialize(get_option('sfsi_plus_section2_options',false));
	...
	$up_option8=  maybe_unserialize(get_option('sfsi_plus_section8_options',false));
...
function sfsi_plus_Unistall_plugin()
{
    ...
    delete_option("sfsi_plus_dismiss_woocommerce");
    delete_option("sfsi_plus_dismiss_twitter");
...
function sfsi_plus_curl_enable_notice(){
	...
	echo '<div class="error"><p> '.__('Error: It seems that CURL is disabled on your server. Please contact your server administrator to install / enable CURL.','ultimate-social-media-plus').'</p></div>'; die;
    }
}
...
function sfsi_plus_activation_msg() {
	...
	if ( $sfsi_plus_activate == 1 && $path['basename'] !== "admin.php?page=sfsi-plus-options" ) {
		...
		echo sprintf(
			__( 'Thank you for installing the %1$sUltimate Social Media PLUS%2$s Plugin. Please go to the %3$splugin\'s settings page%4$s to configure it.', 'ultimate-social-media-plus' ),
			'<b>',
			'</b>',
...
function sfsi_plus_rating_msg() {
	...
	{ ?>
	 ...
	 <div class="sfsi_plus_sfwp_fivestar notice notice-success">
    	<p><?php echo __('We noticed you\'ve been using the Ultimate Social Media PLUS Plugin for more than 40 days. If you\'re happy with it, could you please do us a BIG favor and let us know ONE thing we can improve in it?', 'ultimate-social-media-plus');?></p>
        <ul class="sfwp_fivestar_ul">
        	<li><a href="https://wordpress.org/support/plugin/ultimate-social-media-plus#new-topic-0" target="_new" title="<?php echo __('Yes, let me give feedback.','ultimate-social-media-plus'); ?>"><?php echo __('Yes, let me give feedback.', 'ultimate-social-media-plus'); ?></a></li>
            <li><a href="https://wordpress.org/support/plugin/ultimate-social-media-plus/reviews/?filter=5" target="_new" title="<?php echo __('No clue, let me give a 5-star rating instead','ultimate-social-media-plus') ?>"><?php echo __('No clue, let me give a 5-star rating instead','ultimate-social-media-plus') ?></a></li>
            <li><a href="javascript:void(0);" class="sfsiHideRating" title="<?php echo __('I already did', 'ultimate-social-media-plus')?>"> <?php echo __('I already did (don\'t show this again)', 'ultimate-social-media-plus'); ?> </a></li>
        </ul>
        <button type="button" class="sfsi_plus_plg-rating-dismiss"><span class="screen-reader-text"><?php echo __('Dismiss this notice.','ultimate-social-media-plus'); ?></span></button>
    </div>
    <script>
...
function sfsi_plus_was_displaying_addthis(){
    ...
    $sfsi_plus_section8 =  maybe_unserialize(get_option('sfsi_plus_section8_options',false));
