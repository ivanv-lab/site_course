...
function sfsi_plus_social_buttons_below($content)
{
	...
	if ($sfsi_section8['sfsi_plus_recttwtr'] == 'yes') {
		...
		$icons .= "<div class='sf_twiter' style='display: inline-block;vertical-align: middle;width: auto;'>" . sfsi_twitterShare( $permalink, $twitter_text, 'no' ) . $count_html . "</div>";
	}
	if ($sfsi_section8['sfsi_plus_rectpinit'] == 'yes') {
		...
		$icons .= "<div class='sf_pinit' style='display: inline-block;text-align:left;vertical-align: middle;'>" . sfsi_plus_pinterest_Custom($permalink, $show_count) . $count_html . "</div>";
	}
	$icons .= "</div>";
...
function sfsi_twitterShare( $permalink, $tweettext, $wrapper_div='yes' ) {
...
function sfsi_plus_footer_script()
{
	...
	<?php
		...
		if (!is_admin()) {
			...
			add_action( 'wp_footer', 'sfsi_plus_forum_notification_popup' );
		}
		if (is_admin()) {
	...
	<?php
			...
			if ($sfsi_plus_responsive_icons['settings']['icon_width_type'] === "Fixed icon width") {
				...
				if ( isset( $_GET['page'] ) && 'sfsi-plus-options' === strtolower($_GET['page']) ) {
					$sfsi_plus_anchor_div_style .= 'width:100%;';
				} else {
					$sfsi_plus_anchor_div_style .= 'width:auto;';
				}
			}
			...
			foreach ($sfsi_plus_responsive_icons['default_icons'] as $icon => $icon_config) {
                ...
                $icon = $icon_config['text'] == 'Tweet' ? 'Post on X' : $icon_config["text"];
				$icons .= "\t\t\t\t<span style='color:#fff' >" . $icon . "</span>" . "\n";
				$icons .= "\t\t\t</div>" . "\n";
				$icons .= "\t\t</a>" . "\n\n";
			}
